            ...
            for extension in &client_hello.extensions {
                match extension {
                    Extension::SupportedEllipticCurves(e) => {
                        ...
                        // Pick the first curve we actually support, not blindly
                        // the client's first (modern Chrome offers a post-quantum
                        // hybrid curve first, which we don't recognize). Falls back
                        // to the default if none intersect.
                        state.named_curve = e
                            .elliptic_curves
                            .iter()
                            .copied()
                            .find(|c| !matches!(c, NamedCurve::Unsupported))
                            .unwrap_or(DEFAULT_NAMED_CURVE);
                    }
                    Extension::UseSrtp(e) => {
