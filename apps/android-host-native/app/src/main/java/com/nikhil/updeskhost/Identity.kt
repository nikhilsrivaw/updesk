...
class Identity private constructor(
    ...
    companion object {
        private const val PREFS = "updesk"
        ...
        /** Fixed unattended password — the credential a controller uses to connect
         *  any time (like the native Windows host). Generated once, editable. */
        fun getPassword(ctx: Context): String {
            val prefs = ctx.getSharedPreferences(PREFS, Context.MODE_PRIVATE)
            var pw = prefs.getString("unattended-pw", null)
            if (pw.isNullOrEmpty()) { pw = generatePassword(); prefs.edit().putString("unattended-pw", pw).apply() }
            return pw
        }
        ...
        fun setPassword(ctx: Context, pw: String) {
            ...
            ctx.getSharedPreferences(PREFS, Context.MODE_PRIVATE).edit().putString("unattended-pw", clean).apply()
        }

        fun regeneratePassword(ctx: Context): String {
            val pw = generatePassword()
            ctx.getSharedPreferences(PREFS, Context.MODE_PRIVATE).edit().putString("unattended-pw", pw).apply()
            return pw
        }
