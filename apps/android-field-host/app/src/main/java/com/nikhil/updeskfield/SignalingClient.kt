...
/**
 * WebSocket signaling client — identical JSON protocol to the desktop/host app.
 *
...
class SignalingClient(
    private val url: String,          // wss://up-desk.online
    ...
    // pingInterval keeps the WebSocket alive so idle/hiccup drops don't silently
    // kill the session.
    ...
    fun register() = send(
        JSONObject().put("type", "register").put("deviceId", identity.id)
            .put("metadata", JSONObject().put("os", "android").put("app", "updesk-field"))
    )
    ...
    // End-to-end verification: our signed DTLS fingerprint so the controller can
    // confirm the encrypted channel terminates at us, not a relay/server.
    fun sendE2E(sessionId: String, fp: String, sig: String, pub: String) = send(
        JSONObject().put("type", "e2e").put("sessionId", sessionId)
            .put("fp", fp).put("sig", sig).put("pub", pub)
    )
